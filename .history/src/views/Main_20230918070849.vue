...
const changeFavorites = (todo) => {
  ...
  let favs = getItem('favoriteTodos');

  if (!favs) {
    setItem('favoriteTodos', favorites.value);
    console.log('favs -> ', favorites.value);
    return;
  }

  // if (!todo.isFavorite) {
  //   favs = favs.filter((fav) => fav !== todo.id);
  //   console.log('favs val -> ', favs);
  //   return;
  // }
  // setItem('favoriteTodos', favorites.value);
  ...
  console.log('favorites.value -> ', typeof favorites.value);
