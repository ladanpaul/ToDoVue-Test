...
const { user, todos, filteredUserIdOptions } = storeToRefs(mainStore);

const { setItem, getItem, mergeItem } = useStorage();
...
const setFavorites = (todo) => {
  favorites.value = [...favorites.value, todo.id];
  const fav = getItem('favoriteTodos');

  if (!fav) {
    setItem('favoriteTodos', formData);
  } else {
    mergeItem('favoriteTodos', formData);
  }
};
